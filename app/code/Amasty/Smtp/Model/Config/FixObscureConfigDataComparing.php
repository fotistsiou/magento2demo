...
namespace Amasty\Smtp\Model\Config;
...
/**
 * Preference for Magento SaveConfig class.
 * Fix Magento bug with check changed settings after save config (obscure type)
 * @see \Amasty\Smtp\Model\Config\FixObscureConfigDataComparing::isValueChanged
 * @bug https://github.com/magento/magento2/issues/37392
 */
class FixObscureConfigDataComparing extends \Magento\Config\Model\Config
{
    /**
    ...
    private function isValueChanged(array $oldConfig, string $path, array $fieldData): bool
    {
        if (isset($oldConfig[$path]['value'])) {
            $result = !isset($fieldData['value']) || $this->compareValues($oldConfig, $path, $fieldData);
        } else {
    ...
    private function compareValues(array $oldConfig, string $path, array $fieldData): bool
    {
        if ($path !== 'amsmtp/smtp/oauth2_microsoft_client_secret') {
            return $oldConfig[$path]['value'] !== $fieldData['value'];
        } elseif (preg_match('/^\*+$/', $fieldData['value'])) {
            return false;
        }
